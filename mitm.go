...
var (
    ...
    Four    float64
    Five    float64
    u       = url.URL{Scheme: "ws", Host: "192.168.0.100:12345", Path: "/"}
...
func tojson(resp *http.Response) *http.Response {
    ...
    if len(Char) > 0 {
        WSSend(Char[:len(Char)-2])
    }
    WSSend(fmt.Sprintf("1 SSR(5*) Probability:     %.6f%%", Once/counter*100))
    WSSend(fmt.Sprintf("2 SSR(5*) Probability:     %.6f%%", Twice/counter*100))
    WSSend(fmt.Sprintf("3 SSR(5*) Probability:     %.6f%%", Thrice/counter*100))
    WSSend(fmt.Sprintf("4 SSR(5*) Probability:     %.6f%%", Four/counter*100))
    WSSend(fmt.Sprintf(">5 SSR(5*) Probability:    %.6f%%", Five/counter*100))
    WSSend(fmt.Sprintf("Total: %.6f%%　　    Draw num: %.0f", Total/counter*100, counter))

    fmt.Println("Data Sent via Websocket.")
...
func main() {
    ...
    proxy.OnRequest(goproxy.ReqHostMatches(regexp.MustCompile("^.*yeshen.com$"))).
        HandleConnect(goproxy.AlwaysReject)
